...
def launch_setup(context, *args, **kwargs):
    ...
    num_rover = int(LaunchConfiguration('num_rover').perform(context))
    ...
    for i in range(num_drone):
        ...
        drone_process_list.append(tag_kalman_pos_node)
        ...
        spawn_entity_node = Node(
            ...
            arguments=['-file', f'/tmp/model_{i}.sdf', '-entity', f'robot_{i}', '-x', f'{-5}', '-y', f'{-3}' ],
            output='screen')
    ...
    for i in range(num_rover):
        tag_kalman_pos_node = Node(
            ...
            output='screen',
            parameters=[{'use_sim_time': True},
                        {'system_id': sys_id_count + i + 1},
                        {'robot_type' : robot_type},],
        )
        rover_process_list.append(tag_kalman_pos_node)

        jinja_cmd = [
            f'{gazebo_classic_path}/scripts/jinja_gen.py',
            f'{current_package_path}/models/{robot_type}_uwb/{robot_type}_uwb.sdf.jinja',
            f'{current_package_path}',
            '--mavlink_tcp_port', f'{4560+i+sys_id_count}',
            '--mavlink_udp_port', f'{14560+i+sys_id_count}',
            '--mavlink_id', f'{1+i+sys_id_count}',
            '--gst_udp_port' , f'{5600+i+sys_id_count}' ,
            '--video_uri', f'{5600+i+sys_id_count}',
            '--mavlink_cam_udp_port', f'{14530+i+sys_id_count}',
            '--output-file', f'/tmp/model_{i+sys_id_count}.sdf'
        ]
        jinja_process = ExecuteProcess(
            cmd=jinja_cmd,
            output='screen',
        )
        rover_process_list.append(jinja_process)

        # node to spawn robot model in gazebo
        spawn_entity_node = Node(
            package='gazebo_ros',
            executable='spawn_entity.py',
            arguments=['-file', f'/tmp/model_{i+sys_id_count}.sdf', '-entity', f'robot_{i+sys_id_count}', '-x', f'{3}', '-y', f'{-3 + 6*i}'],
            output='screen')
        rover_process_list.append(spawn_entity_node)
        
        # PX4
        # build_path/bin/px4 -i $N -d "$build_path/etc" >out.log 2>err.log &
        cmd = [
            'env', 
            'PX4_SIM_MODEL=gazebo-classic_rover',
            f'{px4_src_path}/build/px4_sitl_default/bin/px4',
            '-i', f'{i+sys_id_count}',
            '-d',
            f'{px4_src_path}/build/px4_sitl_default/etc',
            '-w', f'{px4_src_path}/build/px4_sitl_default/ROMFS/instance{i+sys_id_count}',
            '>out.log', '2>err.log',
        ]
        px4_process = ExecuteProcess(
            cmd=cmd,
        ...
        rover_process_list.append(px4_process)
        
        start_mission_node = Node(
            package='mission',
            #executable='start_mission_hobber',
            #executable='start_mission_straight',
            executable='start_mission_corner',
            name='start_mission',
            parameters=[{'system_id': i + 1 + sys_id_count},
                        {'robot_type':robot_type},],
            output='screen',
        )
        start_mission_nodes.append(start_mission_node)
...
def generate_launch_description():
    ...
    declared_arguments.append(
        DeclareLaunchArgument(
            'num_rover',
            default_value='1',
            description='Number of rover to spawn'
        )
    )
